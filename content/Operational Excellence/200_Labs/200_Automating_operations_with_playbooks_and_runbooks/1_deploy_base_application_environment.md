...
Next, run the build script to build and deploy you application environment from the Cloud9 workspace as follows:
  ...
  3. Your environment will bootstrap the lab repository. You should see a terminal output showing the following output: Once it is complete you will see a folder called `aws-well-architected-labs`. 
  ...
  4. In the IDE Terminal console, change directort to the working folder where the build script is located:
  ...
  6. The above command runs the build and provisioning of the application stack. When the script should take about 20 mins to finish.
...
Once the application is deployed successfully, go to your [CloudFormation console](https://console.aws.amazon.com/cloudformation/home?region=ap-southeast-2) and locate the stack named `walab-ops-sample-application`.
  ...
  3. Take note of the DNS value specified under **OutputPattern1ApplicationEndpoint**  of the Outputs.
    ...
    The diagram below shows the output from the CloudFormation stack:

    ![Section2 DNS Output](/Operations/200_Automating_operations_with_playbooks_and_runbooks/Images/section2-dns-outputs.png)

  4. Check for the emails on the address you've specified, in the **NotificationEmail** parameter.

  5. Click `confirm subscription` to start subscribe to the application alarm.
...
1. In the **Cloud9** terminal, run the command below, replacing the `ApplicationEndpoint` with the **OutputPattern1ApplicationEndpoint** from previous step. This command will run [curl](https://curl.se/) to send a POST request with the secret message payload `{"Name":"Bob","Text":"Run your operations as code"}` to the API.
